...
import { NAV_ITEMS, HOME_PATH } from '../data/nav-config.js';
...
export function renderFooter({ lang, otherLangPath }) {
  ...
  const { youtube, facebook, instagram, linkedin, whatsapp } = SITE_CONTENT.social;
  const homePath = HOME_PATH[lang];
  
  const navLinks = [
    `<li><a href="${homePath}">${copy.homeLabel}</a></li>`,
    ...NAV_ITEMS.map((item) => `<li><a href="${item[lang].path}">${item[lang].label}</a></li>`)
  ].join('');

  return `<footer class="site-footer"><div class="shell footer-inner">
      <div class="footer-brand">
        <a class="footer-logo" href="${homePath}" aria-label="${copy.brandLabel}">
          <img src="/assets/brand/logo-pas.webp" width="2027" height="1077" alt="PAS Philippe Andy Shembo" loading="lazy">
        </a>
      </div>
      ...
      <div class="footer-socials" aria-label="${copy.socialLabel}">
        <a href="${youtube}" target="_blank" rel="noopener noreferrer" aria-label="YouTube" title="YouTube">
          <svg width="22" height="22" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M23.498 6.186a3.016 3.016 0 0 0-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 0 0 .502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 0 0 2.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 0 0 2.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z"/></svg>
        </a>
        <a href="${facebook}" target="_blank" rel="noopener noreferrer" aria-label="Facebook" title="Facebook">
        ...
        <a href="${whatsapp}" target="_blank" rel="noopener noreferrer" aria-label="WhatsApp" title="WhatsApp">
      ...
      <div class="footer-bottom">
        <p class="footer-rights">${copy.rights}</p>
