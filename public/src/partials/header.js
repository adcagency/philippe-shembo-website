...
export function renderHeader({ lang, pageKey }) {
  ...
  const announcements = lang === 'fr' ? [
    {
      text: "Recevez chaque semaine des encouragements et messages d'édification.",
      cta: "Rejoindre sur WhatsApp"
    },
    {
      text: "Prédications, pensées inspirantes et actualités du ministère en direct.",
      cta: "Suivre le canal officiel"
    }
  ] : [
    {
      text: "Receive weekly encouragements and messages of edification.",
      cta: "Join on WhatsApp"
    },
    {
      text: "Sermons, inspiring thoughts and ministry updates directly on your phone.",
      cta: "Follow official channel"
    }
  ];
  ...
  const slides = announcements.map((item, idx) => `
      <a href="${whatsapp}" target="_blank" rel="noopener noreferrer" class="announcement-slide${idx === 0 ? ' active' : ''}"${idx !== 0 ? ' aria-hidden="true" tabindex="-1"' : ''}>
        ${whatsappIcon}
        <span class="announcement-text">${item.text}</span>
        <span class="announcement-cta">${item.cta} <span aria-hidden="true">→</span></span>
      </a>`).join('');

  return `<div class="announcement-bar" data-announcement-bar>
    <div class="announcement-track">${slides}
    </div>
  </div>
