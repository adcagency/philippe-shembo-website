...
test('bibliography page has one h1, seventeen book cards with show-more button', () => {
  ...
  for (const lang of ['fr', 'en']) {
    ...
    assert.equal((page.bodyHtml.match(/class="book\b/g) || []).length, 17);
    assert.equal((page.bodyHtml.match(/loading="lazy"/g) || []).length, 17);
